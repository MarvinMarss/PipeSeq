...
def main():
    ...
    input_file = find_latest_matching_file(results_folder, r"GTF_results_fpkm_all(?:_\d+)?\.txt")
    ...
    if merged.empty:
        ...
        def calc_pvalue(row):
            exp_values = list(map(float, row["FPKM_exp"]))
            ctrl_values = list(map(float, row["FPKM_ctrl"]))
            n_exp, n_ctrl = len(exp_values), len(ctrl_values)
            if n_exp < 2 or n_ctrl < 2:
                return 1.0
            if n_exp >= 3 and n_ctrl >= 3:
                p_norm_exp  = shapiro(exp_values)[1]
                p_norm_ctrl = shapiro(ctrl_values)[1]
                if p_norm_exp > 0.05 and p_norm_ctrl > 0.05:
                    return ttest_ind(exp_values, ctrl_values, equal_var=False)[1]
                else:
                    return mannwhitneyu(exp_values, ctrl_values,
