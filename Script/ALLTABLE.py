...
def build_matrices(datasets, auto_normalize: bool):
    ...
    for label, path in datasets:
        ...
        pivot_val.columns = [f"{c} / {label}" for c in pivot_val.columns]
        pivot_p.columns   = [f"{c} / {label}" for c in pivot_p.columns]  # NEW

        wide_blocks.append(pivot_val)
        wide_blocks_p.append(pivot_p)  # NEW
    ...
    combined_p   = pd.concat(wide_blocks_p, axis=1)  # NEW
    ...
    return combined_z, combined_p, M_log2, M_p, LONG  # NEW order
...
def plot_heatmap(combined_z: pd.DataFrame, combined_p: pd.DataFrame | None,
                 alpha: float, out_path_png: str):
    # размеры
    ...
    # аннотации: * там, где значимо
    annot = None
    if combined_p is not None:
        # выровнять индексы/колонки на всякий случай
        P = combined_p.reindex(index=combined_z.index, columns=combined_z.columns)
        mask_sig = (P.values <= alpha) & np.isfinite(P.values)
        annot = np.where(mask_sig, "*", "")
    ...
    ax = sns.heatmap(
        ...
        annot=annot, fmt="",
        annot_kws=dict(ha="center", va="center", fontsize=11, fontweight="bold")
    )
...
class AllTableApp(QWidget):
    ...
    def run_build(self):
        try:
            ...
            combined_z, combined_p, M_log2_norm, M_p_norm, LONG = build_matrices(datasets, auto_normalize)
            genes_all = [str(x) for x in combined_z.index.tolist()]

            # колонки в combined_z это "Condition / Method"
            cols_all = [str(c) for c in combined_z.columns.tolist()]
            cond_set, meth_set = set(), set()
            for col in cols_all:
                if " / " in col:
                    cond, meth = col.split(" / ", 1)
                else:
                    # на всякий случай, если формат иной
                    parts = col.rsplit("/", 1)
                    cond = parts[0].strip() if parts else col
                    meth = parts[1].strip() if len(parts) > 1 else ""
                cond_set.add(cond.strip())
                meth_set.add(meth.strip())

            conds_all = sorted(cond_set)
            meths_all = [d[0] for d in datasets if d[0] in meth_set] or sorted(meth_set)

            # Показываем диалог последним шагом перед построением
            dlg_order = OrderConfigDialog(genes_all, conds_all, meths_all, self)
            if dlg_order.exec() != QDialog.DialogCode.Accepted:
                return
            genes_ord, conds_ord, meths_ord = dlg_order.get_orders()

            # Преобразуем в приоритеты
            g_rank = {g: i for i, g in enumerate(genes_ord)}
            c_rank = {c: i for i, c in enumerate(conds_ord)}
            m_rank = {m: i for i, m in enumerate(meths_ord)}

            # Сортируем строки (гены)
            new_index = sorted(combined_z.index, key=lambda g: (g_rank.get(str(g), 10**9), str(g)))
            combined_z = combined_z.reindex(new_index)
            combined_p = combined_p.reindex(new_index)

            # Сортируем колонки по (Condition, Method)
            def col_key(col: str):
                s = str(col)
                if " / " in s:
                    cond, meth = s.split(" / ", 1)
                    ...
                    parts = s.rsplit("/", 1)
                    cond = parts[0].strip() if parts else s
                    meth = parts[1].strip() if len(parts) > 1 else ""
                return (c_rank.get(cond.strip(), 10**9), m_rank.get(meth.strip(), 10**9), cond, meth)

            new_cols = sorted(combined_z.columns, key=col_key)
            combined_z = combined_z.reindex(columns=new_cols)
            combined_p = combined_p.reindex(columns=new_cols)
            ...
            plot_heatmap(combined_z, combined_p, alpha, heatmap_png)
...
class OrderConfigDialog(QDialog):
    """
    Позволяет вручную упорядочить:
      - Гены (строки)
      - Условия (часть слева в "Condition / Method")
      - Методы (часть справа)
    Поддерживает drag&drop и автопреднастройку.
    """
    def __init__(self, genes: list[str], conditions: list[str], methods: list[str], parent=None):
        super().__init__(parent)
        self.setWindowTitle("Порядок отображения (Z-score таблица)")
        self.resize(760, 520)

        self._genes_src = list(map(str, genes))
        self._conds_src = list(map(str, conditions))
        self._meths_src = list(map(str, methods))

        v = QVBoxLayout(self)

        # Подсказка
        lbl = QLabel("Перетащи элементы мышкой или используй авто-кнопки. Этот порядок будет применён к теплокарте.")
        lbl.setWordWrap(True)
        v.addWidget(lbl)

        # Три списка: Гены / Условия / Методы
        h = QHBoxLayout()

        def _mk_list(title, items):
            box = QVBoxLayout()
            ...
            hb.addWidget(btn_up); hb.addWidget(btn_dn); hb.addStretch(1); hb.addWidget(btn_reset)
            box.addWidget(lw); box.addLayout(hb)
            return lw, btn_up, btn_dn, btn_reset, box

        self.lw_genes, g_up, g_dn, g_reset, genes_box = _mk_list("Гены (строки)", self._genes_src)
        self.lw_conds, c_up, c_dn, c_reset, conds_box = _mk_list("Условия (в колонках слева)", self._conds_src)
        self.lw_meths, m_up, m_dn, m_reset, meths_box = _mk_list("Методы (в колонках справа)", self._meths_src)

        h.addLayout(genes_box, 1)
        h.addLayout(conds_box, 1)
        h.addLayout(meths_box, 1)
        v.addLayout(h)

        # Автопреднастройка
        auto_bar = QHBoxLayout()
        btn_auto_genes = QPushButton("Авто: GATA-числом ↑")
        btn_auto_conds = QPushButton("Авто: dark < light < high light, по времени ↑")
        btn_keep_methods = QPushButton("Авто: методы как в таблице")
        auto_bar.addWidget(btn_auto_genes); auto_bar.addWidget(btn_auto_conds); auto_bar.addWidget(btn_keep_methods)
        auto_bar.addStretch(1)
        v.addLayout(auto_bar)
        ...
        # Локальные функции перемещения/сброса
        def move_sel(lw: QListWidget, delta: int):
            row = lw.currentRow()
            if row < 0: return
            new_row = max(0, min(lw.count()-1, row + delta))
            if new_row == row: return
            it = lw.takeItem(row)
            lw.insertItem(new_row, it)
            lw.setCurrentRow(new_row)

        g_up.clicked.connect(lambda: move_sel(self.lw_genes, -1))
        g_dn.clicked.connect(lambda: move_sel(self.lw_genes, +1))
        c_up.clicked.connect(lambda: move_sel(self.lw_conds, -1))
        c_dn.clicked.connect(lambda: move_sel(self.lw_conds, +1))
        m_up.clicked.connect(lambda: move_sel(self.lw_meths, -1))
        m_dn.clicked.connect(lambda: move_sel(self.lw_meths, +1))

        g_reset.clicked.connect(lambda: self._reset_list(self.lw_genes, self._genes_src))
        c_reset.clicked.connect(lambda: self._reset_list(self.lw_conds, self._conds_src))
        m_reset.clicked.connect(lambda: self._reset_list(self.lw_meths, self._meths_src))

        # Авто-правила
        btn_auto_genes.clicked.connect(self._auto_genes)
        btn_auto_conds.clicked.connect(self._auto_conds)
        btn_keep_methods.clicked.connect(lambda: self._reset_list(self.lw_meths, self._meths_src))

        btn_ok.clicked.connect(self.accept)
        btn_cancel.clicked.connect(self.reject)

    def _list_values(self, lw: QListWidget) -> list[str]:
        return [lw.item(i).text() for i in range(lw.count())]

    def _reset_list(self, lw: QListWidget, items: list[str]):
        lw.clear()
        for s in items:
            lw.addItem(QListWidgetItem(s))

    def _auto_genes(self):
        vals = self._list_values(self.lw_genes)
        # «GATA-12», «RBCS», и т.д. — сортировать по числу, затем прочие
        def key(s: str):
            m = re.search(r"GATA[-_ ]?(\d+)", s, re.IGNORECASE)
            if m:
                return (0, int(m.group(1)))
            return (1, s.lower())
        vals_sorted = sorted(vals, key=key)
        self._reset_list(self.lw_genes, vals_sorted)

    def _auto_conds(self):
        vals = self._list_values(self.lw_conds)

        def regime_rank(s: str):
            t = s.lower()
            if "high light" in t: return 2
            if "light" in t:      return 1
            if "dark" in t:       return 0
            return 3

        def time_hours(s: str):
            m = re.search(r"(\d+(?:[.,]\d+)?)\s*(?:h|hour|hours)\b", s, re.IGNORECASE)
            if m:
                try: return float(m.group(1).replace(",", "."))
                except: return float("inf")
            return float("inf")

        vals_sorted = sorted(vals, key=lambda s: (regime_rank(s), time_hours(s), s.lower()))
        self._reset_list(self.lw_conds, vals_sorted)

    def get_orders(self) -> tuple[list[str], list[str], list[str]]:
        return (
            self._list_values(self.lw_genes),
            self._list_values(self.lw_conds),
            self._list_values(self.lw_meths),
        )
# ---------- Точка входа ----------
