...
def parse_count_info(count_files):
    samples = []
    for count_file in count_files:
        if not (count_file.startswith("gene_counts_") and count_file.endswith(".txt")):
            continue
        full_sample = count_file[len("gene_counts_"):-len(".txt")]
        info = parse_sample_name(full_sample)
...
def main():
    ...
    if skip_counting:
        sample_df = parse_count_info(count_files)
        if sample_df.empty:
            log("В папке Counts не найдены count-файлы для анализа.")
            sys.exit(1)
        log(f"Используем {len(sample_df)} count-файлов из папки Counts.")
    else:
