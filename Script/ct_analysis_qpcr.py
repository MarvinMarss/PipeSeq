...
class CtAnalysisApp(QWidget):
    ...
    def compute_statistics(self):
        ...
        def ref_series(raw_ct, lab, ref_list):

            series = [raw_ct[lab].get(r, []) for r in ref_list]
            series = [s for s in series if s]
            if not series:
                return []
            L = min(len(s) for s in series)

            return [float(np.nanmean([s[k] for s in series])) for k in range(L)]
        ...
        for exp_num, cond in uniq:
            ...
            for gene in genes:
                if gene in ref_genes:  # исключаем все Control-* из таргетов
                    continue

                d_ctrl = []
                for lab in ctrls:
                    rvals = ref_series(raw_ct, lab, ref_genes)
                    gvals = raw_ct[lab][gene][:len(rvals)]
                    d_ctrl += [x - y for x, y in zip(gvals, rvals)]

                d_exp = []
                for lab in exps:
                    rvals = ref_series(raw_ct, lab, ref_genes)
                    gvals = raw_ct[lab][gene][:len(rvals)]
                    d_exp += [x - y for x, y in zip(gvals, rvals)]
