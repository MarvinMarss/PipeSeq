...
if cfg.get("generate_histograms", False):
    ...
    for gene in gene_order:
        ...
        pvals = annotations[gene].values

        plt.figure(figsize=(8, 5))
        plt.bar(range(len(conditions)), values, yerr=pvals, capsize=5, edgecolor='black')
...
if cfg["show_p_values"]:
    def format_pvalue(log2_val, p_val):
        ...
        return f"{log2_val:.1f}\n({p_val:.2f})" if p_val >= 0.059 else f"*{log2_val:.1f}*\n({p_val:.2f})"
...
if cfg.get("remove_color_for_high_p", False):
    ...
    for text in ax.texts:
        ...
        if row < heatmap_data.shape[0] and col < heatmap_data.shape[1]:
            if annotations.iloc[row, col] > 0.059:
